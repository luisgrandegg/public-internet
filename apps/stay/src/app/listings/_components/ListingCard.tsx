...
interface ListingCardProps {
   ...
   * Pass null (or omit) for signed-out visitors — the save toggle is not rendered.
   */
...
export function ListingCard({ listing, favorited = null, onMouseEnter, onMouseLeave }: ListingCardProps) {
  const firstPhoto = listing.photos[0]

  return (
    <div className={styles.wrapper} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <Link href={`/listings/${listing.id}`} className={styles.root}>
