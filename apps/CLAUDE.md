...
├── src/
│   ├── __generated__/
│   │   └── prisma/        ← generated by prisma generate — gitignored, never edit
│   └── app/
...
Each app must generate its Prisma client to its own `src/__generated__/prisma/` directory — **never to the shared pnpm `@prisma/client` location**. Two apps generating to the same shared location creates a race condition where the last one to run overwrites the other's types.

**Required schema generator config:**
...
**Required import in `src/lib/db.ts`:**

```typescript
import { PrismaClient } from '@/__generated__/prisma'
```

**Required `.gitignore` entry in the app root:**

```
src/__generated__/
```

The `postinstall` script (`prisma generate`) will regenerate it on every `pnpm install`.
